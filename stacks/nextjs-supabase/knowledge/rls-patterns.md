...
## Gotchas
- **`service_role` bypass RLS** — **SECURITY WARNING: `service_role` bypasses ALL RLS policies. Only use server-side in admin operations. Never expose to client.** Le client `service_role` ignore toutes les policies sans exception.
