...
Each wave can write checkpoint data for resume/recovery:

```json
"checkpoints": {
  "wave_3": {
    "features_done": ["F01", "F02"],
    "features_failed": [],
    "features_pending": ["F03", "F04"],
    "worktrees_active": [],
    "last_checkpoint_at": "2026-04-01T10:30:00Z"
  }
}
```

### Checkpoint Rules
- Written after each feature completes in Wave 3
- Updated atomically (same file lock as orchestre.lock)
- `features_done` is append-only within a run
- `features_failed` triggers retry (max 1) before marking `skipped`
- On resume, orchestrator reads checkpoints to skip completed features
