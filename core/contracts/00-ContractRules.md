...
### Architecture Rules (aligned with CLAUDE.base.md — source of truth)

| Rule | Description | Scope | Hook |
|------|-------------|-------|------|
| R1 | Business logic in dedicated logic layer only — no DB/fetch in routing or components | `app/`, `components/`, `src/routes/` | TS-03 |
| R2 | Components = pure UI — data via props, no data fetching | `components/`, `src/lib/components/` | TS-03 |
| R3 | Types derived from schemas (Zod/Pydantic) — never manual duplicates | `**/*.ts`, `**/*.tsx` | — |
| R4 | Errors as Result\<T\> — never throw in business logic layer | `lib/`, `src/lib/server/` | TS-02 |
| R5 | 1 feature = 1 isolated folder — no cross-feature imports | `components/`, `src/lib/components/` | IMPORT-02 |
| R6 | Server-first rendering (stack-specific: Server Components / +page.server.ts) | `app/`, `src/routes/` | — |
| R7 | Mutations via server-side patterns only (Server Actions / form actions) | `app/`, `src/routes/` | — |
| R8 | No magic strings — use typed constants | `**/*.ts`, `**/*.tsx` | — |

Additional hook-enforced checks (not architecture rules but coding standards):
| Check | Description | Hook |
|-------|-------------|------|
| No `any` type | Use `unknown` + narrowing | TS-01 |
| Semantic tokens only | No hardcoded Tailwind colors | DESIGN-01 |
| Import aliases | Use `@/` or `$lib/` — no deep relative paths | IMPORT-01 |
| No secrets in code | API keys, JWTs, tokens blocked | SECRET-01..10 |
| No sensitive console.log | User data in logs blocked | SECRET-09 |
