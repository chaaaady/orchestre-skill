...
export const secretsChecker = {
  ...
  async check(filePath, content) {
    ...
    const patterns = [
      ...
      { rule: 'SECRET-02', regex: /eyJ[a-zA-Z0-9_-]{10,}\.[a-zA-Z0-9_-]{10,}\.[a-zA-Z0-9_-]{10,}/, message: 'JWT token detected' },
    ...
    // Check for NEXT_PUBLIC_ or PUBLIC_ (SvelteKit) on secret variable names
    const secretVarPattern = /(NEXT_PUBLIC_|PUBLIC_)(STRIPE_SECRET|SUPABASE_SERVICE_ROLE|DATABASE_URL|.*_SECRET_KEY|.*_API_KEY(?!.*ANON))/i;
