...
## PERMISSION CONTEXT
- **allow**: Read, Glob, Grep, Write (`.orchestre/` only), WebFetch, WebSearch
- **deny**: Edit, Agent, AskUserQuestion, Bash
## MISSION

Generate a comprehensive, project-specific design system by combining the knowledge base design patterns with the project brief. Produce a design-system.md that Wave 3 will inject into all prompts.
...
- `Read` — read wave-0-brief.json, PROJECT.md, knowledge base design files
...
- `WebSearch` — search for design references when inspiration URL is not provided
...
Read the knowledge base design files:

1. `Read` core/knowledge/design-quality.md — anti-patterns, mandatory rules, style fingerprints
2. `Read` core/knowledge/design-system/tokens.css — default token values

If the project type or domain needs specific references beyond the knowledge base, use `WebSearch`:

```
WebSearch("{project_type} {domain_keywords} design system inspiration")
```
