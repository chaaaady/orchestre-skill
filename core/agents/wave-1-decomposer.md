...
- `Read` — read PROJECT.md, wave-0-brief.json, knowledge files
...
```json
{
  ...
  "category": "auth|billing|ui|api_endpoint|data_model|integration|infrastructure|design_system",
...
│                                   ┘
└── F07 (SEO) — depends on all "critical" features
```
...
parallel_groups:2
estimated_cost:$4.50
intent_hash:sha256:{hash of orchestre.intent.json}
```

**Integrity (CR-03):** After writing `orchestre.intent.json`, compute its SHA-256 hash and record it in both `WAVE_1_DONE` and `orchestre.lock` under `hashes.intent`. Wave 2 MUST verify this hash before consuming the intent.
